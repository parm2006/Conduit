...
A third authenticated control candidate waits while the Server UI explains the two-Client limit and offers:

- replace Client 1;
- replace Client 2;
- reject the newcomer.

Replacement cleanly releases the old Client's input, cancels its transfers, disconnects its lanes, and removes it from active routing. The newcomer inherits the freed placement when its display shape fits; otherwise it appears unplaced in the draft.
...
Cancel discards the draft, restores the active arrangement in the editor, and dismisses Client draft toasts. Leaving the editor or moving Server-controlled focus away from it has the same discard behavior.
...
5. If valid, release all injected input, return the roaming cursor to the Server primary-display center, pause routing, reject new file jobs, and quiesce active file scheduling.
6. Preserve the newest ordinary clipboard snapshot and revision.
7. Distribute the candidate topology to all ready Clients.
8. Require acknowledgements from every participant.
9. Atomically activate and persist the graph, then resume routing, transfers, and clipboard delivery.
10. If distribution or acknowledgement fails, restore and resume the previous active graph.

Client draft toasts disappear after commit or rollback/cancel.
...
- A pending third Client cannot displace an existing session without the Server user's explicit choice.
...
- Session registry capacity, authentication, lane binding, mismatched lanes, timeout, replacement, and isolated disconnect.
...
- Draft/active separation, Cancel, invalid Apply, acknowledgement failure, atomic commit, persistence, and rollback.
...
- Three-source clipboard receive ordering, latest-wins replacement, single-use echo suppression, stale revision rejection, late-Client sync, and ordinary copies during file transfer.
...
- Compact GUI rendering/state tests: no add/detect control, in-grid Apply/Cancel, fixed Server, automatic Client cells, initials, color stability, invalid red Clients, and no user-facing IP.
...
- Third-Client replace/reject flow.
...
- Rapid screenshots during a large file transfer.
...
- Use one shared Server password without weakening per-device trust.
